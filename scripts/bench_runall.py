...
def add_args(parser: ArgumentParser):
    parser.add_argument(
        "--depths", type=int, default=[64], nargs="+", help="Queue depth to test"
    )
    parser.add_argument(
        "--sizes", type=int, default=[4096], nargs="+", help="I/O sizes to test"
    )
    parser.add_argument(
        "--numcpus_range",
        type=int,
        default=None,
        nargs="*",
        help="Range of how many CPUs to test",
    )
    parser.add_argument(
        "--numcpus_specific",
        type=int,
        default=None,
        nargs="*",
        help="The amount of CPUs to test",
    )
    parser.add_argument(
        "--numdevs_range",
        type=int,
        default=None,
        nargs="*",
        help="Range of how many devices to test",
    )
    parser.add_argument(
        "--numdevs_specific",
        type=int,
        default=None,
        nargs="*",
        help="The amount of devices to test",
    )
    parser.add_argument(
        "--cpu_freqs",
        type=str,
        default=["ondemand"],
        nargs="*",
        help="List of fixed CPU frequencies (in GHz) and governors to test",
    )
    parser.add_argument(
        "--turbo",
        type=int,
        default=[0, 1],
        nargs="+",
        help="0 for turbo boost off, 1 for turbo boost on, [0,1] for testing both",
    )
    parser.add_argument(
        "--smt",
        type=int,
        default=[0, 1],
        nargs="+",
        help="0 for SMT off, 1 for SMT on, [0,1] for testing both",
    )
    parser.add_argument(
        "--hyperthreads",
        type=int,
        default=[0, 1],
        nargs="+",
        help="0 for hyper threads off, 1 for hyper threads on, [0,1] for testing both. Note that you cannot test with hyper threads if SMT is turned off",
    )
    parser.add_argument(
        "--stress",
        type=int,
        default=[0, 1],
        nargs="+",
        help="0 for not stressing unused CPUs, 1 for stressing unused CPUs, [0,1] for testing both",
    )
    parser.add_argument(
        "--time", type=int, default=5, help="Time for each benchmark run"
    )
    parser.add_argument(
        "--fio_size", type=str, default="16GiB", help="Working-set size for fio_xnvme"
    )
    parser.add_argument(
        "--results_dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="The amount of times each benchmark will be repeated. The result will be average of the repetitions",
    )
    parser.add_argument(
        "--nqueues",
        type=int,
        default=[1],
        nargs="+",
        help="Number of queues per device (used by xnvmeperf-cuda)",
    )
    parser.add_argument(
        "--rws",
        type=str,
        default=["randread"],
        nargs="+",
        help="List of I/O patterns to test",
    )
    parser.add_argument(
        "--tool",
        choices=[
...
def main(args, cijoe: Cijoe):
    ...
    if args.tool != "xnvmeperf-cuda":
        if (args.numcpus_range and args.numcpus_specific) or (
            not args.numcpus_range and not args.numcpus_specific
        ):
            log.error(
                "Failed: You must define exactly 1 of arguments: numcpus_range, numcpus_specific"
            )
            return 1

    if (args.numdevs_range and args.numdevs_specific) or (
        not args.numdevs_range and not args.numdevs_specific
    ):
        log.error(
            "Failed: You must define exactly 1 of arguments: numdevs_range, numdevs_specific"
        )
        return 1
    ...
    benchmarker = BenchHelper(
        cijoe, bdev_configs, bdev_results, cfm, args.tool, args.backend, args.fio_size
    )
    ...
    if args.tool == "xnvmeperf-cuda":
        ...
        for devs, iosz, qd, nq in tests:
            ...
            for i in range(args.repetitions):
                err, result = benchmarker.run_benchmark(
                    "randread", qd, iosz, devs, 0, args.time, "N/A", f"-{i}", nq
                )
                if err:
    ...
    tests = [
        (ht, tu, sm, st, f, d, c, rw, o, q)
        for (ht, tu, sm, st, f, d, c, rw, o, q) in tests
        if not (not sm and ht)
    ]
    ...
    for ht, tu, sm, st, freq, devs, cpus, rw, iosz, qd in tests:
        ...
        for i in range(args.repetitions):
            err, result = benchmarker.run_benchmark(
                rw, qd, iosz, devs, cpus, args.time, freq, f"{suffix}-{i}"
            )
            if err:
                log.error("Failed: run_benchmark()")
                return err
...
def create_range(default: Optional[list], arr: list) -> range:
    ...
    if 1 <= default[0] <= len(arr):
        ...
        log.error(
            f"Error: given start-index out of range({default[0]}); using 1 as start-index"
        )
    ...
    if 1 <= default[1] <= len(arr):
        ...
        log.error(
            f"Error: given end-index out of range({default[1]}); using length of arr as end-index"
        )
...
def print_progress(finished: int, total: int, duration_s: float):
    ...
    stderr.write(
        f"\r{bar}  {finished} / {total} {remaining_string if int(duration_s) else ''}  "
    )
    if finished == total:
        stderr.write("\n")
    stderr.flush()
